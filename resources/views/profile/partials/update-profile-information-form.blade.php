...
<form method="post" action="{{ route('profile.update') }}" class="space-y-4">
    ...
    <div>
        ...
        <input id="name" name="name" type="text" value="{{ old('name', $user->name) }}"
            ...
            class="block w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent">
        @error('name')
        ...
        @if ($user instanceof \Illuminate\Contracts\Auth\MustVerifyEmail && ! $user->hasVerifiedEmail())
            <div class="mt-2">
                <p class="text-sm text-gray-600">
                    ...
                    <button form="send-verification" class="text-green-600 hover:underline text-sm">
