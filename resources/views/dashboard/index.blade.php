                ...
                @foreach(['today' => "Aujourd'hui", 'week' => 'Semaine', 'month' => 'Mois', 'quarter' => 'Trimestre'] as $p => $label)
                    <button onclick="changePeriod('{{ $p }}')"
                       ...
                       class="period-btn px-3 py-1.5 text-xs font-medium rounded-md transition
                              {{ $period === $p ? 'bg-white shadow-sm text-orange-700' : 'text-gray-500 hover:text-gray-700' }}">
                        {{ $label }}
                    </button>
                @endforeach
            </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <a href="{{ route('conversations.index', ['status' => 'open']) }}"
               class="bg-white rounded-xl p-5 border border-gray-200 hover:border-green-300 hover:shadow-md transition group">
                <div class="flex items-center gap-3">
                    <div class="w-10 h-10 bg-orange-100 rounded-lg flex items-center justify-center group-hover:bg-green-200 transition">
