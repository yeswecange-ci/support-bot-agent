...
<div class="flex-1 overflow-y-auto bg-gray-50">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 py-6">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-6">
            ...
            <div class="bg-white rounded-xl border border-gray-200 p-4 text-center">
                <p class="text-2xl font-bold text-orange-600">{{ number_format($totalStats['active'] ?? 0) }}</p>
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-5 mb-6">
            <form method="GET" action="{{ route('bot-tracking.conversations') }}" class="flex flex-wrap items-end gap-3">
                <div class="flex flex-col gap-1 flex-1 min-w-48">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Nom, telephone..." class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-orange-500 focus:border-green-500 bg-white">
                </div>
                <div class="flex flex-col gap-1">
                    <label class="text-xs font-medium text-gray-600">Statut</label>
                    <select name="status" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-orange-500 focus:border-green-500 bg-white">
                ...
                <div class="flex flex-col gap-1">
                    ...
                    <select name="is_client" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-orange-500 focus:border-green-500 bg-white">
                        <option value="">Tous</option>
                    ...
                    <tbody class="divide-y divide-gray-50">
                        ...
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-4 py-3 text-sm text-gray-500">
                                @if($conv->started_at)
                                    {{ \Carbon\Carbon::parse($conv->started_at)->format('d/m/Y H:i') }}
                                @else
                                    —
                                @endif
                            </td>
